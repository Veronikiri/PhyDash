...
public:
    ...
    virtual ~Player() = default;   // ВИРТУАЛЬНЫЙ ДЕСТРУКТОР — ОБЯЗАТЕЛЬНО!
    ...
    void resetPosition() { m_shape.setPosition({ 200.f, 525.f }); m_velocity = { 450.f, 0.f }; m_isJumping = false; }
    ...
    void setColors(sf::Color primary, sf::Color secondary, sf::Color glow, bool glowEnabled) {
        m_primaryColor = primary; m_secondaryColor = secondary;
        m_glowColor = glow; m_glowEnabled = glowEnabled;
    }
...
protected:
    ...
    sf::Vector2f m_velocity = { 450.f, 0.f };
